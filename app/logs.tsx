...
export default function LogsScreen() {
  ...
  // Simple grouping by day (naive approach for UI demo)
  const groupedLogs = logs.reduce((acc, log) => {
    // Basic date parsing assumption: log.time contains YYYY-MM-DD
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.detailCard}>
            <View style={styles.detailHeader}>
              ...
              <Text style={styles.detailTitle}>{lang === 'zh-CN' ? '打卡详情' : 'Check-in Detail'}</Text>
            ...
            <View style={styles.successIconContainer}>
              ...
              <Text style={styles.successText}>{lang === 'zh-CN' ? '打卡成功' : 'Success'}</Text>
            </View>

            <View style={styles.detailList}>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>{lang === 'zh-CN' ? '课程' : 'Course'}</Text>
                <Text style={styles.detailValue}>{courseName}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>{lang === 'zh-CN' ? '成员' : 'Member'}</Text>
                <Text style={styles.detailValue}>{memberName}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>{lang === 'zh-CN' ? '变动' : 'Change'}</Text>
                <Text style={styles.detailValue}>-1 {lang === 'zh-CN' ? '课时' : 'Session'}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>{lang === 'zh-CN' ? '时间' : 'Time'}</Text>
                <Text style={styles.detailValue}>{selectedLog.time}</Text>
              </View>
              <View style={styles.detailRow}>
                <Text style={styles.detailLabel}>{lang === 'zh-CN' ? '备注' : 'Note'}</Text>
              ...
              return (
                <TouchableOpacity key={log.id} style={styles.logItem} onPress={() => setSelectedLog(log)}>
                  ...
                  <View style={styles.contentContainer}>
                    ...
                    <Text style={styles.subText}>{memberName || 'Member'} · -1 {classItem?.unitType === 'session' ? '次' : '课时'}</Text>
                  </View>
                  <View style={styles.rightContainer}>
                    <Text style={styles.timeText}>{log.time.split(' ')[1] || log.time}</Text>
                    {classItem && (
                       <Text style={styles.subText}>{lang === 'zh-CN' ? `剩余 ${classItem.totalLessons - classItem.doneLessons}` : `Rem ${classItem.totalLessons - classItem.doneLessons}`}</Text>
                    )}
                  </View>
                </TouchableOpacity>
              );
...
const styles = StyleSheet.create({
  ...
  logItem: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', padding: 16, borderRadius: 20, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 8, elevation: 2 },
