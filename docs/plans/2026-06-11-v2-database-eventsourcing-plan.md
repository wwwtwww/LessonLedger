# V2 Event-Sourced Database Implementation Plan

> **For Gemini:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Rebuild the Supabase database to fully implement the Event-Sourced architecture with idempotency and multi-tenant RLS defined in the new PRD.

**Architecture:** Create a unified DDL script (`01_event_sourced_v2.sql`). The `logs` table becomes append-only with three delta fields (`done_lesson_delta`, `total_lesson_delta`, `price_delta`). PostgreSQL triggers handle state projection onto the `classes` table. Unique partial indexes prevent double-reversals and duplicate check-ins for the same schedule instance.

**Tech Stack:** PostgreSQL, Supabase.

---

### Task 1: Create the DDL Script for Event-Sourced Schema

**Files:**
- Modify: `supabase/migrations/00_init_v2.sql` (We will overwrite the previous iteration of this file entirely).

**Step 1: Write the minimal implementation**

We overwrite `supabase/migrations/00_init_v2.sql` with the new schema defined in the PRD, replacing the old draft.
...
git commit -m "chore(db): overwrite schema for event-sourced logs and 3-dimensional deltas"
```

---

### Task 2: Implement Constraining Indexes & Event-Sourcing Trigger

**Files:**
- Modify: `supabase/migrations/00_init_v2.sql`

**Step 1: Write the constraints and trigger function**

Append the idempotency constraints and the trigger logic to the end of the file. The trigger simply aggregates the deltas on insert.
...
```bash
git add supabase/migrations/00_init_v2.sql
git commit -m "chore(db): add unique constraints and append-only projection trigger"
```

---

### Task 3: Implement Auth RPCs & RLS

**Files:**
- Modify: `supabase/migrations/00_init_v2.sql`

**Step 1: Write the RPCs and RLS Policies**

Append the invite code RPCs and tenant isolation RLS policies to the end of the file.
...
```bash
git add supabase/migrations/00_init_v2.sql
git commit -m "chore(db): add rpc auth helpers and robust multi-tenant RLS"
```
